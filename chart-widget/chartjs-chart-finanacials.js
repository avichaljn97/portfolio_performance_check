...
class CandlestickElement extends FinancialElement {
	draw(ctx) {
		...
		if (close < open) {
			...
			borderColor= "rgba(121, 166, 55, 1)";
			ctx.fillStyle="rgba(121, 166, 55, 1)";
			...
			borderColor = "rgba(  191, 11, 44, 1)";
			ctx.fillStyle = "rgba(  191, 11, 44, 1)";
		} else {
		...
		ctx.beginPath();
		ctx.moveTo(x, high);//high wick
		ctx.lineTo(x, Math.min(open, close));
		ctx.moveTo(x, low);//low wick
		ctx.lineTo(x, Math.max(open, close));
		ctx.stroke();
		
		ctx.fillRect(x - me.width / 2, close, me.width , open - close);//main candle
